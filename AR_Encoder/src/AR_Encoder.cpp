...
* 文件名称 : AR_Encoder.h
* 版    本 : V1.0
* 说    明 : 软件解码初始化，共4个连接Encoder转接板的PORT，
*			 每个连接端口有5个引脚:DIR_A1,DIR_A2,PWM,ENCODER_A1,ENCODER_A2:电机A的两个方向控制引脚，PWM控制引脚，encode输出引脚
*			 还有个INT引脚练到单片机外部中断，也不知道干啥用的
* Copyright (C), 2015-2020, 阿波罗科技 www.apollorobot.cn
*
*********************************************************************************************************
*/

AR_Encoder::AR_Encoder(uint8_t s1, uint8_t s2)
{
	s1_pin = s1;
	s2_pin = s2;
...
int16_t AR_Encoder::calcMotorSpeed()
{
	
}

int16_t AR_Encoder::calcMotorAverageSpeed()
{
	
}

int AR_Encoder::calcMotorMileaga()
{
	
}

/*******************************************************************************
* Function Name  : AR_Encoder
* Description    : 
* Input          : None
* Output         : None
* Return         : None
 ...
 */
AR_Encoder::AR_Encoder(uint8_t slot)
{
  _Slot = slot;
 ...
 *   None
 */
void AR_Encoder::reset(uint8_t slot)
{
  _Slot = slot;
  _Port_A = encoder_Port[slot].port_A;
  _Port_B = encoder_Port[slot].port_B;
 ...
 *   None
 */
uint8_t AR_Encoder::getSlotNum(void)
{
  return _Slot;
}
