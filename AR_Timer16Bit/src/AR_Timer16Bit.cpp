...
* 版    本 : V1.0
* 说    明 : Atmega328P只有一个16bit定时器/计数器 Timer1
*
...
AR_Timer16Bit Timer16Bit;              // preinstatiate
unsigned char AR_Timer16Bit::instances = 0;
...
void AR_Timer16Bit::attachInterrupt(void (*isrOvf)(), void (*isrCapt)(), long microseconds)
{
  ...
  if(NULL != isrOVFCallBack)  TIMSK1 |= 1<<TOIE1;         // sets the timer overflow interrupt enable bit
  if(NULL != isrCAPTCallBack) {
	  TIMSK1 |= 1<<ICIE1;		                          // sets the timer captrue  interrupt enable bit
	  TCCR1B |= (1 << ICNC1) | (1 << ICES1);          // Enable Noise Canclation & Set the Rising edg detect
  }
